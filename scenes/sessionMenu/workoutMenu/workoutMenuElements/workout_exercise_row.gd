...
func _populate_sets() -> void:
	...
	for set_index in range(sets.size()):
		...
		# Pass sub menu container
		row.sub_menu_container = sub_menu_container
		# Pass the set data, exercise index, and set index to the row
		row.setup(set_data, exercise_index, set_index)
...
func _on_add_set_pressed() -> void:
	...
	# You might want to use default values from the exercise
	var default_weight = 0.0
	var default_reps = 0
	
	# If exercise has a rep range, use the minimum as default
	if exercise_data and exercise_data.exercise:
		default_reps = exercise_data.exercise.rep_range.x
...
func _on_exercise_changed(new_exercise_name: String) -> void:
	...
	if exercise_data:
		...
		# Clear sets and let user add new ones
		...
		print("Exercise changed to: ", exercise.name)
...
func _on_set_added(exercise_idx: int, set_index: int) -> void:
	...
	if exercise_idx == exercise_index:
		# Refresh the sets for this exercise
		_populate_sets()
...
func _on_set_updated(exercise_idx: int, set_index: int) -> void:
	...
	if exercise_idx == exercise_index:
		...
		# We can find the child at set_index and call refresh() on it
